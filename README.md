...
Loads the student dataset from data.py.
Splits the data into training and testing sets.
Scales the features (study_hours, attendance, past_marks) using StandardScaler.
Trains a LinearRegression model on the scaled training data.
Evaluates the model's performance on the test data and prints the MAE and R2 score.
Predicts results for a predefined set of new student profiles.
Prompts the user to enter custom student data to generate a new prediction and a Pass/Fail result.
Example Interaction
$ python main.py
MAE: 1.15
R2 Score: 0.99
